"""
Evaluate fine-tuned model (base + LoRA adapter) on the same eval sample
used by the baseline benchmark, so results are directly comparable.
"""
...
SAMPLE_SIZE = 50  # same seed as baseline so it's the same 50 examples


def load_finetuned_model(base_model: str, adapter_dir: Path):
    logger.info(f"Loading base model {base_model} …")
    tokenizer = AutoTokenizer.from_pretrained(adapter_dir)  # saved alongside adapter
    base = AutoModelForCausalLM.from_pretrained(
        base_model,
        torch_dtype=torch.float32,
        low_cpu_mem_usage=True,
    )
    logger.info(f"Merging LoRA adapter from {adapter_dir} …")
...
def run():
    ...
    model, tokenizer = load_finetuned_model(MODEL_NAME, ADAPTER_DIR)
    ...
    logger.info("\n" + "=" * 60)
    logger.info("FINE-TUNED RESULTS")
    logger.info("=" * 60)
    ...
    out = BENCHMARK_DIR / "finetuned_metrics.json"
...
if __name__ == "__main__":
    run()
    print("\n✓ Evaluation complete — see benchmarks/finetuned_metrics.json")
